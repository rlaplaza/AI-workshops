...
- It uses the rows in the BH dataset as the full BO scope (no synthetic expansion).
- It starts from low-performing seed experiments to show recovery behavior.
- It compares two uncertainty-aware models:
  1) Gaussian Process Regressor
  2) Random Forest with tree-ensemble uncertainty proxy
- It runs a simple expected-improvement BO loop and reveals true yields on demand.
"""
...
# ------------------------------- User settings ------------------------------- #
CSV_PATH = Path(__file__).with_name("BH_HTE_small.csv")
TARGET_COL = "Yield"
N_BO_STEPS = 10
RANDOM_STATE = 7
...
def build_initial_low_yield_seed(df: pd.DataFrame, feature_cols: list[str]) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if len(seed_df) < N_INITIAL_LOW_POINTS:
        # Backfill if dedup removed rows with repeated feature tuples.
...
def lookup_true_yield(
    ...
    """Reveal the true objective value from dataset for the chosen condition."""
...
def main() -> None:
    ...
    # 3) Use dataset rows directly as the BO universe.
    print_banner("STEP 3 - DATASET-SCOPED POOL (NO SYNTHETIC EXPANSION)")
    universe_df = df[feature_cols + [TARGET_COL]].drop_duplicates(subset=feature_cols, keep="first").reset_index(
        drop=True
    )
    ...
    # 4) Build initial seed and remaining candidate pool.
    ...
    print(
        f"Initial low-yield seed size: {len(observed_df)} "
        f"(non-zero points in seed: {n_nonzero_seed})"
    )
    print(
        f"Constraint check: at least {MIN_NONZERO_LOW_POINTS} non-zero low-yield points "
        f"=> {n_nonzero_seed >= MIN_NONZERO_LOW_POINTS}"
    )
    ...
    print(
        ...
        "- choose_batch_and_record: select top-k EI candidates + reveal true yields\n"
    ...
    def choose_batch_and_record(
        ...
        for batch_rank, (_, chosen_local) in enumerate(batch.iterrows(), start=1):
            ...
            observed_yield_local = lookup_true_yield(
                all_data=universe_df,
    ...
    for step in range(1, N_BO_STEPS + 1):
        ...
        print(
            f"Revealed true yields range: {min(batch_yields):.3f} to {max(batch_yields):.3f}"
        )
    ...
    print(
        ...
        "- True values were revealed only after selection and then appended to training."
    )
    ...
    print("Tip: this script simulates sequential experimentation by hiding true yields until queried.")
